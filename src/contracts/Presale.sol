...
contract Presale is IPresale, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        if (_liquidityLocker == address(0) || _vestingContract == address(0)) revert InvalidInitialization();
    ...
    function _liquify(uint256 _currencyAmount, uint256 _tokenAmount) private {
        if (_currencyAmount == 0 || _tokenAmount == 0) revert ZeroLiquidityAmounts();
    ...
    function _validateContribution(address _contributor, uint256 _stablecoinAmountIfAny) private view {
        ...
        if (contributions[_contributor] + amount > options.max) revert ExceedsMaximumContribution();
    }
